...
### 0110 — высота папок без поля пересылки

- Панель папок поднимается только когда показано поле комментария. При пустом
  поле она остаётся у нижнего края; отступ списка пересчитывается при анимации.

### 0111 — видимый ID профиля

- В профилях людей, групп и каналов, а также в своём профиле и настройках
  аккаунта, ID показан отдельной строкой. Нажатие копирует его. Формат для
  групп и каналов совпадает с выбранным в настройках Telegram/Bot API.

### 0112 — медиаменю, стекло и инструменты сообщений

- PhotoViewer передаёт меню собственный источник размытия. ActionBarMenuItem
  сохраняет уже назначенную фабрику вместо повторной записи всего окна.
- BottomSheet оборачивает фон материалом NebulaSheetSurface с сохранением
  исходных отступов и запасным обычным фоном. Эффект требует API 31+ и включённого
  размытия LiteMode. MainTabsLayout не кеширует живой фон hardware-слоем и
  сбрасывает состояние на detach.
- LaunchActivity обрабатывает ссылки `tg://settings/nebula` после проверки
  блокировки приложения. Ссылки выполняют только навигацию.
- ChatActivity открывает инструменты выбранного сообщения и отменяет очередь
  автоперевода на pause. TranslateController использует отдельный добровольно
  включаемый перевод; штатное выключение и выбор языка также обслуживают его.
- ChatMessageCell показывает раскрываемую заглушку фильтра, включая доступность.
  UserConfig возвращает десять аккаунтов без зависимости от Premium.
- Manifest регистрирует внутренний приёмник напоминаний, восстанавливающий
  расписание после перезагрузки. Подробнее: `docs/ANDROID-CHAT-TOOLS.md`.

### 0114 — скрытие NebulaLink на вложенных экранах

- Значок NebulaLink добавляется в шапку списка чатов. Когда поверх него
  открывается список тем или другой вложенный экран, значок скрывается на время
  перехода и возвращается после закрытия. Состояние и подключение NebulaLink не
  затрагиваются.
