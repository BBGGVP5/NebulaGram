...
- `FloatingToolbar.createContentContainer`: без внешней фабрики размытия панель получает `NebulaMenuBackdrop` — тот же снимок окна-источника, что и остальные всплывающие меню. Панель живёт в отдельном окне, своего фона у неё нет.
- `STYLE_BLACK` (поверх фотографии) остаётся плоским: под панелью там не обои, а сама фотография.
- Проверка: применение цепочки; рендер на устройстве — отдельно.
