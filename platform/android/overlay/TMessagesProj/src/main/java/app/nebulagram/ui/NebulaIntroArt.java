...
public final class NebulaIntroArt extends View {
    ...
    private void welcome(Canvas c) {
        card(c,42,18,283,319,24);
        text(c,"NebulaGram",62,51,18,ink,true);
        pill(c,60,68,114,92,t("Все","All"),cyan);
        text(c,t("Личные   Работа","Personal   Work"),128,84,11,muted,false);
        ...
        for(int i=0;i<3;i++) {
            ...
            text(c,names[i],101,y-3,12,ink,true);
            text(c,messages[i],101,y+15,10,muted,false);
        }
        pill(c,60,272,172,299,t("Твои разговоры","Your conversations"),cyan);
        // The mascot occupies the empty right side, never the preview text.
        sticker(c,226,136,356,330);
    }
    private void design(Canvas c) {
        int[] colors={0xff4fc7c9,0xff7565ee,0xffdd91c4,0xff438adf,0xff365172,0xff758acf};
        String[] icons={"✦","◈","Aa","☷","N","▣"};
        for(int i=0;i<6;i++) {
            int x=42+(i%3)*96,y=22+(i/3)*104;
            gradient(c,x,y,x+84,y+84,24,colors[i],0xff19253b);
            text(c,icons[i],x+(i==2?15:25),y+55,i==2?31:40,ink,true);
        }
        ribbon(c,36,244,324,294,"✦",t("Папки, темы и иконки","Folders, themes, icons"));
    }
    private void privacy(Canvas c) {
        gradient(c,64,16,296,320,28,0xff435678,0xff202a3b);
        text(c,t("Код доступа","Passcode"),123,53,16,ink,true);
        text(c,"••••",143,95,30,ink,true);
        for(int i=0;i<9;i++) {
            int x=118+(i%3)*62,y=134+(i/3)*51;
            circle(c,x,y,20,0x203cd4d4);text(c,""+(i+1),x-5,y+6,17,ink,false);
        }
        circle(c,180,287,20,0x203cd4d4);text(c,"0",175,293,17,ink,false);
        // A compact badge sits outside the keypad rather than across its keys.
        circle(c,293,55,27,0xff496ddb);
        p.setStyle(Paint.Style.STROKE);p.setStrokeWidth(3);p.setColor(ink);
        r.set(284,39,302,59);c.drawRoundRect(r,9,9,p);p.setStyle(Paint.Style.FILL);
        r.set(280,51,306,69);c.drawRoundRect(r,5,5,p);
    }
