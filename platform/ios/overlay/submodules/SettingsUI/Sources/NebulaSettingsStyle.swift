...
enum NebulaSettingsStyle {
    ...
    static func icon(symbol: String, color: UIColor? = nil) -> UIImage? {
        guard let image = UIImage(systemName: symbol,
                withConfiguration: UIImage.SymbolConfiguration(pointSize: 19, weight: .regular)) else { return nil }
        let accent = color ?? self.accent(for: symbol)
        return UIGraphicsImageRenderer(size: CGSize(width: 32, height: 32)).image { _ in
            accent.setFill()
            UIBezierPath(roundedRect: CGRect(x: 0, y: 0, width: 32, height: 32), cornerRadius: 9).fill()
            let scale = min(21 / image.size.width, 21 / image.size.height)
